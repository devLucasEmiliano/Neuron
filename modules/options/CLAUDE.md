...
- **Modelos (Textos)**: Per-category text model editor. Supports string-valued models (key + textarea) and object-valued models (nested key/sub-key textareas). Keys can be renamed; models can be added/removed and reset to default. A "placeholder panel" at the top of each category lists the substitution keys (e.g., `{NUP}`, `{PRAZO}`) available for that category, sourced from `shared/js/text-placeholders.js`; chips are click-to-copy.
...
- `shared/js/text-placeholders.js` — `window.NEURON_TEXT_PLACEHOLDERS` provides the list of substitution keys rendered in the "Modelos de Texto" placeholder panel.
