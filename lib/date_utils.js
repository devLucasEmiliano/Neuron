...
window.DateUtils = (function() {
    ...
    const logger = window.NeuronLogger ? window.NeuronLogger.createLogger('date_utils') : null;
    ...
    (async function carregarConfiguracoes() {
        try {
            ...
            if (!chrome?.storage?.local) {
                if (logger) {
                    logger.warning("chrome.storage não disponível. Usando configurações padrão");
                } else {
            ...
            if (result.neuronUserConfig) {
                ...
                if (logger) {
                    logger.info("Regras e feriados carregados", { globalRules, holidays: holidays.length });
                } else {
                    console.log("DATE_UTILS v4.0: Regras e feriados carregados.", { globalRules, holidays: holidays.length });
                }
            } else {
                if (logger) {
                    logger.warning("Configuração não encontrada. Usando valores padrão");
                } else {
            ...
            if (logger) {
                logger.error("Falha crítica ao carregar configurações", error);
            } else {
    ...
    function ajustarDataFinal(data, ruleOverrides = {}) {
        ...
        if (holidayRule !== 'none') {
            ...
            if (tentativas >= maxTentativas) {
                if (logger) {
                    logger.warning('Limite de tentativas atingido ao ajustar feriados. Retornando data sem ajuste completo');
                } else {
    ...
    function adicionarDiasUteis(dataInicial, dias) {
        ...
        if (tentativas >= maxTentativas) {
            if (logger) {
                logger.warning('Limite de tentativas atingido ao calcular dias úteis. Resultado pode estar incompleto');
            } else {
